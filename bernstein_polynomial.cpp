  ...
  for (i = 0; i <= n; i++){
    ...
    for ( j = 0; j <= n; j++ ){
      ...
      for ( k = 0; k <= i; k++){
          ...
          a[i+j*(n+1)] = a[i+j*(n+1)] + r8_mop(i + k)*pow(r8_choose(i,k), 2)/r8_choose(n+i, j+k);
      }

      a[i+j*(n+1)] = a[i+j*(n+1)]*r8_choose(n, j)*(double)(2*i+1)/(double)(n+i+1);
    }
  }
...
double *bernstein_to_power(int n){
//
//  Purpose:
//
...
//
  ...
  for(j = 0; j <= n; j++){
      for(i = 0; i <= j; i++){
          a[i+j*(n+1)] = r8_mop(j-i)*r8_choose(n-i, j-i)/r8_choose(n, i);
    }
  }
...
double *legendre_to_bernstein(int n){
//
//  Purpose:
//
...
//
  ...
  for(i = 0; i <= n; i++){
    for(j = 0; j <= n; j++){
      for(k = i4_max(0, i + j - n); k <= i4_min(i, j); k++){

        a[i+j*(n+1)] = a[i+j*(n+1)] + r8_mop(j+k)*pow(r8_choose(j, k), 2)*r8_choose(n-j, i-k);
      }

        a[i+j*(n+1)] = a[i+j*(n+1)]/r8_choose(n, i);
    }
  }
...
double *power_to_bernstein(int n){
//
//  Purpose:
//
...
//
  ...
  for(j = 0; j <= n; j++){
      for(i = 0; i <= j; i++){
          ...
          a[(n-1)+(n-j)*(n+1)] = r8_choose(j, i)/r8_choose(n, i);
    }
  }
